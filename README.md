...
- **趋势分析**：自动检测趋势变化点
- **非负值约束**：支持 CPU、内存等非负指标的强制约束
  - 自动将输入数据中的负值截断为 0
  - Prophet 模型添加 floor=0 约束
  - 预测结果中的负值自动截断为 0
- **数据清洗**：自动忽略包含 null 值的数据点
...
| `enforce_non_negative` | true | 强制非负值（CPU/内存） |

### 非负指标处理

对于 **CPU、内存、磁盘使用率** 等非负指标，启用 **强制非负值** 选项可确保：

1. **数据预处理**：输入数据中的负值自动设为 0
2. **模型约束**：Prophet 添加 `floor=0` 约束
3. **结果后处理**：预测值中的负值自动截断为 0

这样可以避免模型因数据噪声产生不合理的负值预测。
...
### 2026-02-06 - 数据清洗增强
- 自动忽略 JSON 数据中的 null 值
- 过滤不完整的数据点（缺失时间戳或数值）

### 2026-02-06 - 非负值支持
- 添加非负值约束功能（适用于 CPU、内存等指标）
- 数据预处理自动截断负值为 0
- Prophet 模型添加 floor 参数约束
- 预测结果后处理确保非负
